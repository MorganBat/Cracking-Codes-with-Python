...
def removeSolvedLettersFromMapping(letterMapping):
    ...
    # maps to ['N'], the we know that 'B' must map to 'N', so we can
    # removed 'N' from the list of what 'A' could map to. So 'A' then maps
...
def hackSimpleSub(message):
    ...
    for cipherword in cipherwordList:
        ...
        # Intersecr the new mapping with the existing intersected mapping:
...
def decryptWithCipherLetterMapping(ciphertext, letterMapping):
    ...
    for cipherletter in LETTERS:
        if len(letterMapping[cipherletter]) == 1:
            ...
            ciphertext = ciphertext.replace(ciphertext.lower(), '_')
            ciphertext = ciphertext.replace(ciphertext.upper(), '_')
    key = ''.join(key)
